...
Pull Request 使用隔离的 `ubuntu-latest`；可信 main push 整合队友的 Linux x64 `self-hosted` Runner 模式。这里没有机械覆盖队友的单体 CI：如果原样覆盖，会丢失三个业务服务独立测试、85 个接口、微服务 Gateway 和微服务 Kubernetes 清单。

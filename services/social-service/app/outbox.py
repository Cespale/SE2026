...
from .clients import get_content_client, get_user_client
...
async def deliver(event: IntegrationOutbox) -> None:
    ...
    if event.event_type == "video.interaction-counts.changed":
        await get_content_client().request_json(
            "PUT",
            f"/internal/videos/{event.payload['videoId']}/interaction-counts",
            request_id,
            json=event.payload,
        )
        return

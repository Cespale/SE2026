...
# 应用各服务清单（替换版本占位符）
for f in k8s/postgres.yaml k8s/minio.yaml k8s/backend.yaml k8s/frontend.yaml k8s/srs.yaml; do
...
kubectl -n "${NAMESPACE}" rollout status deploy/streamhub-srs --timeout=180s
